...
lazyaircrack
printf ${Blue}"\n\n════════════════════════════════╣ "$Yellow"Netowk Information"${Blue}" ╠════════════════════════════════\\n\n\n"${White}

ip addr show 2>/dev/null
printf ${Blue}"\n\n════════════════════════════════╣ "$Yellow"SysInformation"${Blue}" ╠════════════════════════════════\\n\n\n"${White}
...
if [ "$dddd" ]; then
  echo "\e[92mKernel :\e[00m $dddd"

else 
  :
fi
DOCKER_CONTAINER=`grep -i docker /proc/self/cgroup  2>/dev/null; grep -i docker /proc/1/cgroup 2 > /dev/null; find / -maxdepth 3 -name "*dockerenv*" -exec ls -la {} \; 2>/dev/null`
if [ "$DOCKER_CONTAINER" ]; then   echo "\e[41mDocker Container\e[0m"; 
else echo "\e[92mNot in a Docker Container\e[0m"; fi;
LXC_CONTAINER=`grep -qa container=lxc /proc/1/environ 2>/dev/null`
if [ "$LXC_CONTAINER" ]; then echo "\e[41mLXC Container"; else echo "\e[92mNot in a LXC container\e[0m"; fi
sleep 2
...
echo "\e[93m[+] Useful file locations:\e[00m" ; which nc 2>/dev/null ; which netcat 2>/dev/null ; which wget 2>/dev/null ; which nmap 2>/dev/null ; which gcc 2>/dev/null; which curl 2>/dev/null 
echo "\n" 
pathinfo=`echo $PATH 2>/dev/null`
if [ "$pathinfo" ]; then
  pathswriteable=`ls -ld $(echo $PATH | tr ":" " ")`
  echo "\e[93m[+] Path information:\e[00m\n$pathinfo" 
  echo  "$pathswriteable"
  echo  "\n"
fi
...
echo "\e[93m[+] Finding emails inside logs: \e[00m\n"
...
echo "\e[93m[+] Finding IPs inside logs:\e[00m\n"
...
if [ "$SudoUser" ]; then
  echo  "\e[92m[+] Root users : \e[00m$SudoUser" 
  echo  "\n"
fi
roothist=`ls -la /root/.*_history 2>/dev/null`
if [ "$roothist" ]; then
  echo "\e[93m[+] Root's history files are accessible: \e[00m\n$roothist" 
  echo "\n"
fi



echo  "\n\e[93m[+] Can we read/write sensitive files:\e[00m" ; ls -la /etc/passwd 2>/dev/null ; ls -la /etc/group 2>/dev/null ; ls -la /etc/profile 2>/dev/null; ls -la /etc/shadow 2>/dev/null ; ls -la /etc/master.passwd 2>/dev/null 
echo "\n" 

#lists available shells
shellinfo=`cat /etc/shells 2>/dev/null`
if [ "$shellinfo" ]; then
  echo  "\e[93m[+] Available shells:\e[00m\n$shellinfo" 
  echo  "\n"
fi
...
if [ "$crontabvalue" ]; then
  echo "\e[93m[+] Crontab contents:\e[00m\n$crontabvalue" 
  echo "\n"
fi

crontabvar=`ls -la /var/spool/cron/crontabs 2>/dev/null`
if [ "$crontabvar" ]; then
  echo  "\e[93m[+] Anything interesting in /var/spool/cron/crontabs:\e[00m\n$crontabvar" 
  echo  "\n"
fi

anacronjobs=`ls -la /etc/anacrontab 2>/dev/null; cat /etc/anacrontab 2>/dev/null`
if [ "$anacronjobs" ]; then
  echo  "\e[93m[+] Anacron jobs and associated file permissions:\e[00m\n$anacronjobs" 
  echo  "\n"
fi

anacrontab=`ls -la /var/spool/anacron 2>/dev/null`
if [ "$anacrontab" ]; then
  echo  "\e[93m[+] When were jobs last executed (/var/spool/anacron contents):\e[00m\n$anacrontab" 
  echo  "\n"
fi
...
if [ "$cronother" ]; then
  echo  "\e[93m[+] Jobs held by all users:\e[00m\n$cronother" 
  echo  "\n"
fi
